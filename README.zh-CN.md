...
本项目处于发布前活跃开发阶段。本地桌面管理流程已端到端实现，并保留明确的安全确认和可见的失败状态。公开远程管理、RCON 和静默自动内容安装仍明确不在范围内。

## 界面架构

- 仪表盘聚合全部本地服务器、生命周期状态、备份和待处理事项。
- 每台服务器进入可深链的八区工作台：总览、控制台、玩家、内容、文件与备份、运行状况、自动化和服务器设置。
- 控制台输出、生命周期变化、玩家状态与性能采样共用桌面事件通道；仅仪表盘摘要和当前服务器状态保留有界的兜底轮询。
- 创建服务器与已安装内容管理共用同一个市场浏览器。
- Minecraft 结构化文件优先使用表格或表单编辑器；不支持的格式仍保留高级源码编辑。
- 整个界面完整支持英文和简体中文。
